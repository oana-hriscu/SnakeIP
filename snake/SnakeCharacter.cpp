...
void SnakeCharacter::grow()
{
	...
	case Movement::Up_Arrow:
		nodes.push_back(Node(sf::Vector2f(nodes[nodes.size() - 1].getPos().x, nodes[nodes.size() - 1].getPos().y + Node::Height)));
		break;
	case Movement::Down_Arrow:
		nodes.push_back(Node(sf::Vector2f(nodes[nodes.size() - 1].getPos().x, nodes[nodes.size() - 1].getPos().y - Node::Height)));
		break;
	case Movement::Left_Arrow:
		nodes.push_back(Node(sf::Vector2f(nodes[nodes.size() - 1].getPos().x + Node::Width, nodes[nodes.size() - 1].getPos().y)));
		break;
